...
export class EstudoLivreComponent implements OnInit {
  ...
  infoGramatica(template: TemplateRef<any>){
    this.modalInfoRef = this.modalService.show(template,Object.assign({}, { class: 'gray modal-lg' }));
  }
  errorGramatica(template: TemplateRef<any>){
    this.modaErrorlRef = this.modalService.show(template,Object.assign({}, { class: 'modal-sm' }));
  }

  
  fecharModal(){
	this.modalInfoRef= this.modalInfoRef!=null? this.limparRefModal(this.modalInfoRef) : null;
	this.modaErrorlRef= this.modaErrorlRef!=null? this.limparRefModal(this.modaErrorlRef) : null;
	this.modaArvoreRef= this.modaArvoreRef!=null? this.limparRefModal(this.modaArvoreRef) : null;
	  ...
	  this.service.arvoreOtimizada(this.request.xml).subscribe(
		response=> {
			if(response['success']){
				...
				this.modaArvoreRef = this.modalService.show(template,Object.assign({}, { class: 'modal-lg', }))
			}	  
		},
		error=>null
	  );
...
iniciarDerivacao(){
	...
	if(this.visualizararvore==true){
		this.service.iniciar(this.request.xml).subscribe(
			response=>{
				if(response['success']){
