...
export class PesquisarExercicioComponent implements OnInit {
  ...
  lista_niveis=null;
  nivelId=null;
  constructor( private service: ExerciciosService,
  ...
  carregarLista(){
    ...
    this.service.listarNiveis().pipe(map((response) => {
      if (response['success']==true) {
          return response['data']
        // ----------------------------------------------------
      }else{
        this.errorMensagem('Ocorreu um error ao buscar os niveis disponiveis');
        return []
      }
    })).subscribe(
