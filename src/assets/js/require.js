...
(function (load) {
    ...
    self.addEventListener(
      ...
      function (evt) {
        if (evt.error instanceof Error) {
          if (pwd[0]) {
            evt.preventDefault();
            throw new evt.error.constructor(
              evt.error.message,
              pwd[0].uri,
  ...
  try {
    ...
    console.warn(
      'Honey: falling back to DOM workaround for cache object (' + e + ')',
    );
  ...
  var requirePath =
    self.Honey && self.Honey.requirePath !== undefined
      ? self.Honey.requirePath.slice(0)
      : ['./'];
  var requireCompiler =
    self.Honey && self.Honey.requireCompiler !== undefined
  ...
  for (var i = 0; i < requirePath.length; i++) {
    parser.href =
      (requirePath[i][0] == '.' ? base[1] : base[0]) + requirePath[i];
  ...
  function require(identifier, callback, compiler) {
    ...
    //      readyState 4.
    callback &&
      (request[request.onload === null ? 'onload' : 'onreadystatechange'] =
        onLoad);
  ...
  function resolve(identifier) {
    ...
    // ==========================================================================================
    parser.href =
      (m[2] ? root + p[2] + m[2] + '/' : root) + m[3] + (m[4] ? m[4] : 'index');
