...
var ATNDeserializationOptions =
  require('./ATNDeserializationOptions').ATNDeserializationOptions;
  ...
  for (var i = 0; i < atn.states.length; i++) {
    ...
    if (state instanceof PlusBlockStartState) {
      ...
      if (state.transitions[0].target instanceof StarBlockStartState) {
        this.checkCondition(
          state.transitions[1].target instanceof LoopEndState,
        );
        this.checkCondition(!state.nonGreedy);
      } else if (state.transitions[0].target instanceof LoopEndState) {
        this.checkCondition(
          state.transitions[1].target instanceof StarBlockStartState,
        );
        this.checkCondition(state.nonGreedy);
      } else {
        throw 'IllegalState';
      ...
      this.checkCondition(
        state.transitions[0].target instanceof StarLoopEntryState,
      );
      ...
      this.checkCondition(
        state.transitions.length <= 1 || state instanceof RuleStopState,
      );
    }
  }
};
...
ATNDeserializer.prototype.edgeFactory = function (
  atn,
  type,
  src,
  trg,
...
ATNDeserializer.prototype.lexerActionFactory = function (type, data1, data2) {
  ...
  if (
    type > this.actionFactories.length ||
